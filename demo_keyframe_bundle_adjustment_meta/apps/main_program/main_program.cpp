...
int main(int argc, char **argv) {
  ...
  if (!n.getParam("viso_config", visoConfigFileName)) {
  ...
  ros::Publisher pub_laser_cloud =
      n.advertise<sensor_msgs::PointCloud2>("/kitti/velo/pointcloud", 2);
  ...
  while (std::getline(timestamp_file, line) && ros::ok()) {
    ...
    cv::Mat left_processed_display = left_processed.clone();
    feature_tracker_module.process(left_processed, timestamp_ros);
    auto match_msg = feature_tracker_module.get_matches_msg();
    feature_tracker_module.reduce();
    cv::imshow("Original", left_image);

    cv::imshow("Processed", left_processed_display);
    cv::waitKey(1);
